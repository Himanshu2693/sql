...
SELECT D1.CNAME FROM DEPOSIT D1, BRANCH B2, D1.BNAME AND B2.CITY IN (SELECT B1.CITY FROM DEPOSIT D2 BRANCH B1 WHERE D2.CNAME='ANIL' AND D2.BNAME=B1.BNAME); 

SELECT D1.ACTNO,D1.BNAME,D1.AMOUNT,D1.ADATE ,BR1.LOANNO, BR1. BNAME BR1.AMOUNT FROM DEPOSIT D1, BORROW BR1, CUSTOMERS C1 WHERE C1.CNAME = D1.CNAME AND D1.CNAME=BR1.CNAME AND C1.CITY IN (SELECT C2.CITY FROM CUSTOMERS C2 WHERE C2.CNAME ='PRAMOD'); 

SELECT D1.CNAME FROM DEPOSIT D1,CUSTOMERS C1 WHERE D1.AMOUNT > 2000 D1.CNAME=C1.CNAME AND C1.CITY IN (SELECT C2.CITY FROM CUSTOMERS C2 WHERE C2.CNAME='ANIL');

SELECT D1.BNAME FROM DEPOSIT D1 GROUP BY D1.BNAME HAVING COUNT(D1.CNAME) > = ALL (SELECT COUNT GROUP BY D2.BNAME);
